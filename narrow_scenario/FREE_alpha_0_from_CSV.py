...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 16 18:57:41 2020
...
"""
 ...
 DESCRIPTION:
    ...
    datasize
        ...
        - it is equal in each node
        - is the buffer of total data to be send
    avgsend
        average sending interval, in miliseconds
    collision
        0   simplified check. Two packets collide when they arrive
            at the same time, on the same frequency and SF
        1   considers the capture effect
        2   consider the Non-orthognality SFs effect and capture effect*
        *capture effect: consider relative time at coming and Prx level
        (perhaps if 2 packets arrive, then stronger is decoded)
    randomseed
        random seed
 OUTPUT
...
#ASUME A BAND
# Bandwidth
#BUT ALSO WILL FIT WITH VALUES: 125 (TYPICAL), 250
Bandwidth = 500
# Coding Rate
CodingRate = 1 ##NO REDUNDACY, NO EXTRA BIT FOR DECODE ALGORITHM
# Duty cycle
Dutycycle = 0.01 ##FROM NORMATIVE

# packet size per SFs
# EACH VALUE IS PER SF
#IS THE PACKAGE LENGTH FOR EACH FRAME, NOTICED THAT BIGGER IS BETTER
PcktLength_SF = [242,246,243,206,245,244]
LorawanHeader = 8
# channels per sf, SEE TABLE IV
# EACH SF HAS ONLY PRESENCE ON 1 CH, EXCEPT SF 11 AND 12
...
def ber_reynders_snr(snr, sf, bw, cr):
    ...
    CR = Temp[cr-1]
    BW = bw*1000.0
    #ES LA FORMULA (3) DEL PAPER, PAG 4
...
#WE ASUME COLLISION, FOR CASE OF 500 OF FREC BW
...
class myNode():
    def __init__(self, nodeid, bs, datasize):
        ...
        rounds = 0
        global nodes
        #NODES DISTRIBUTION ARE PLACED IN A RANDOM WAY
        #ALSO COPUTE DISTANCE
        while (found == 0 and rounds < 100):
            ...
            if len(nodes) > 0:
                ...
                ##MEANS THAT THERE ARE NO NODES YET
                ##FIRST NODE ALSO IS PLACED RANDOMLY
                print ("first node")
                self.x = posx
                self.y = posy
                found = 1
        ...
        # graphics for node
        # GOOD TO KNOW WHERE NODES ARE PLACED
        global graphics
        if (graphics == 1):
            global ax
...
class myDummyNode():
    def __init__(self, nodeid, bs, x, y, dist, datasize):
        ...
        self.y = y
        self.dist = dist
##ALSO FIRST STAGE, GW WAITS FOR JOIN-REQUESTS
class stage1():
    def __init__(self, nodeid, distance):
        ...
        self.nodeid = nodeid
        self.txpow = 14 ##ASUMED 14dBm OF POWER FOR THIS STAGE
        ## assign bandwidth
        self.bw = Bandwidth
        ## assign coding rate
        self.cr = CodingRate
        ## assign spreading factor
        ##SF 12 IS ACCORDING TO DEF IN THE PAPER.
        self.sf = 12  # [default]
...
# is maintained
#HERE WE WILL USE SIMPY TO MODEL TRANSMISSION
...
# global stuff
#DEFINE EMPTY NODES, SO FIRST PRINT WILL BE "first node"
#NODES IS A LIST FILLED WIT EQUAL NUMBER OF NODES PASSED AS INPUT
nodes = []
dummynodes = []
